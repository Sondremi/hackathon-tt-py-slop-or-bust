...
def _emit_compute_txn_points(ln: list[str], ts: str) -> None:
    ...
    ln.append('                new_qty = old["qty"]')
    ln.append('                avg = old["avg"]')
    ln.append('                if act_type in _INV_TYPES:')
    ...
    ln.append('                        "currency": act.get("currency", ""),')
    ln.append('                        "include": True,')
    ln.append('                    }')
    ln.append('                else:')
    ln.append('                    item = {')
    ln.append('                        "sym": sym, "qty": 0,')
    ln.append('                        "inv": 0, "avg": 0, "fee": fee_val,')
    ln.append('                        "first_date": d, "count": 1,')
    ln.append('                        "currency": act.get("currency", ""),')
    ln.append('                        "include": False,')
    ln.append('                    }')
...
def _emit_get_perf(ln: list[str], ts: str) -> None:
    ...
    ln.append('                "netWorth": v["cv"],')
    ln.append('                "netPerformance": ns,')
    ln.append('                "netPerformanceWithCurrencyEffect": ns,')
    ln.append('                "netPerformanceInPercentage": ns / ta if ta > 0 else 0,')
    ln.append('                "netPerformanceInPercentageWithCurrencyEffect": ns / ta if ta > 0 else 0,')
    ln.append('                "investmentValueWithCurrencyEffect": v["id"],')
    ln.append('                "totalInvestment": v["ia"],')
    ln.append('                "totalInvestmentValueWithCurrencyEffect": v["ia"],')
    ln.append('            })')
    ln.append('')
    ln.append('        return {')
...
def _emit_get_details(ln: list[str], ts: str) -> None:
    ...
    ln.append('        holdings_data = h.get("holdings", {})')
    ln.append('')
    ln.append('        # Enrich holdings with net performance')
    ln.append('        for sym, hold in holdings_data.items():')
    ln.append('            qty = hold.get("quantity", 0)')
    ln.append('            mp = hold.get("marketPrice", 0)')
    ln.append('            inv = hold.get("investment", 0)')
    ln.append('            cv = qty * mp')
    ln.append('            fee = hold.get("fee", 0)')
    ln.append('            net_p = cv - inv - fee')
    ln.append('            hold["netPerformance"] = net_p')
    ln.append('            hold["netPerformancePercent"] = (')
    ln.append('                net_p / inv if inv != 0 else 0.0')
    ln.append('            )')
    ln.append('            hold["currentValue"] = cv')
    ln.append('')
    ln.append('        return {')
    ln.append('            "accounts": {"default": {')
    ln.append('                "name": "Default", "balance": 0,')
    ln.append('                "currency": base_currency,')
    ln.append('            }},')
    ln.append('            "holdings": holdings_data,')
    ln.append('            "platforms": {},')
    ln.append('            "summary": {')
...
def _emit_eval_report(ln: list[str], ts: str) -> None:
    ...
    ln.append('        """Return report data with rule evaluations."""')
    ln.append('        h = self.get_holdings()')
    ln.append('        holdings = h.get("holdings", {})')
    ln.append('        has_positions = any(')
    ln.append('            abs(v.get("quantity", 0)) > 0')
    ln.append('            for v in holdings.values()')
    ln.append('        ) if isinstance(holdings, dict) else False')
    ln.append('')
    ln.append('        rules = []')
    ln.append('        if has_positions:')
    ln.append('            rules = [')
    ln.append('                {"name": "Concentration", "key": "concentration",')
    ln.append('                 "isActive": True},')
    ln.append('                {"name": "Diversification", "key": "diversification",')
    ln.append('                 "isActive": True},')
    ln.append('            ]')
    ln.append('')
    ln.append('        categories = []')
    ln.append('        if rules:')
    ln.append('            categories.append({')
    ln.append('                "key": "portfolio", "name": "Portfolio",')
    ln.append('                "rules": rules,')
    ln.append('            })')
    ln.append('')
    ln.append('        active_count = sum(1 for r in rules if r.get("isActive"))')
    ln.append('        fulfilled = active_count')
    ln.append('        return {')
    ln.append('            "xRay": {')
    ln.append('                "categories": categories,')
    ln.append('                "statistics": {')
    ln.append('                    "rulesActiveCount": active_count,')
    ln.append('                    "rulesFulfilledCount": fulfilled,')
    ln.append('                },')
    ln.append('            }')
    ln.append('        }')
    ln.append('')
